...
* Bảng patients
- department_id khóa ngoại patient_id
- patient_id đếm số bệnh nhân

Output
- department_id: Mã khoa
- department_name: Tên khoa
- total_patients: Tổng số bệnh nhân duy nhất thuộc khoa đó
- total_revenue: Tổng số tiền thu được từ các bệnh nhân thuộc khoa đó

Giải pháp
- Dùng Inner join để nối 3 bảng departments patients, invoices lại với nhau dựa trên các cặp khóa ngoại
- Dùng COUNT(DISTINCT patients.patient_id) ể đếm chính xác tổng số bệnh nhân, tránh trường hợp một bệnh nhân có nhiều hóa đơn bị đếm lặp lại nhiều lần
--> Dùng Inner join
*/
-- Tạo view
...
FROM departments d
INNER JOIN patients p ON d.department_id = p.department_id
